...
jshort OSPopShort(OperandStack *s) { return *(--s->next); }

void OSPushShort(OperandStack *s, jshort val) { *s->next++ = val; }

jshort OSGetShort(OperandStack *s) { return *(s->next - 1); }
...
jshort VTGetShort(VariableTable *t, uint8_t index) { return t->base[index]; }
